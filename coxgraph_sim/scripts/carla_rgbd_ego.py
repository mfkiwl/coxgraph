...
class DisplayManager:
    ...
    def get_display_size(self):
        return [
            int(self.window_size[0] / self.grid_size[0]),
            int(self.window_size[1] / self.grid_size[1])
        ]
...
class SensorManager:
    ...
    def init_sensor(self, sensor_type, transform, attached, sensor_options):
        if sensor_type == 'IMU':
            imu_bp = self.world.get_blueprint_library().find(
                'sensor.other.imu')
            #            imu_bp.set_attribute("sensor_tick",str(200.0))
            ego_imu = self.world.spawn_actor(imu_bp,
                                             transform,
                                             attach_to=attached)
        ...
        if sensor_type == 'DepthCamra':
            ...
            depth_cam = self.world.spawn_actor(depth_bp,
                                               transform,
                                               attach_to=attached)
            depth_cam.listen(self.save_depth_image)
            ...
            lidar = self.world.spawn_actor(lidar_bp,
                                           transform,
                                           attach_to=attached)
            ...
            radar = self.world.spawn_actor(radar_bp,
                                           transform,
                                           attach_to=attached)
...
def one_run(args, client):
    ...
    try:
        ...
        SensorManager(world, display_manager, 'RGBCamera',
                      carla.Transform(carla.Location(x=1.5, z=2.4)), vehicle,
                      {}, [0, 0])
        SensorManager(world, display_manager, 'DepthCamra',
                      carla.Transform(carla.Location(x=1.5, z=2.4)), vehicle,
                      {}, [0, 0])
        SensorManager(world, display_manager, 'IMU',
                      carla.Transform(carla.Location(x=1.5, z=2.4)), vehicle,
                      {}, [0, 0])
        ...
        if args.lidar_number >= 3:
            SensorManager(
                world, display_manager, 'LiDAR',
                carla.Transform(carla.Location(x=0, z=2.4)), vehicle, {
                    'channels': '64',
                    'range': '50',
        ...
        if args.semanticlidar_number >= 3:
            SensorManager(
                world, display_manager, 'SemanticLiDAR',
                carla.Transform(carla.Location(x=0, z=2.4)), vehicle, {
                    'channels': '64',
                    'range': '50',
        ...
        if args.radar_number >= 3:
            SensorManager(
                world, display_manager, 'Radar',
                carla.Transform(carla.Location(x=0, z=2.4),
                                carla.Rotation(pitch=5, yaw=90)), vehicle,
        ...
        if args.radar_number >= 1:
            SensorManager(
                world, display_manager, 'Radar',
                carla.Transform(carla.Location(x=0, z=2.4),
                                carla.Rotation(pitch=5)), vehicle,
                {'points_per_second': radar_points_per_second}, [2, 2])
        ...
        while True:
            ...
            if not args.profiling:
                if frame == 30:
                    ...
                    print("FPS: %.3f %.3f %.3f" %
                          (time_frames, 1.0 / time_frames * 30,
                ...
                if (timer.time() - time0) > 10.0:
                    ...
                    prof_str = "%-10s %-9s  %-9s %-15s %-7.2f %-20.3f" % (
                        args.lidar_number, args.semanticlidar_number,
                        args.radar_number, lidar_points_per_second,
...
def main():
    ...
    argparser.add_argument('--no-render-window',
                           action='store_false',
                           dest='render_window',
    ...
    try:
        ...
        if args.profiling:
            print("-------------------------------------------------------")
            print(
                "# Running profiling with %s lidars, %s semantic lidars and %s radars."
                % (args.lidar_number, args.semanticlidar_number,
                   args.radar_number))
            ...
            print(
                "#NumLidars NumSemLids NumRadars PointsPerSecond FPS     PercentageProcessing"
            )
