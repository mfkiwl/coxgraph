...
void CoxgraphServer::mapFusionCallback(
  ...
  if (!needRefuse(cid_a, t1, cid_b, t2)) return;
  ...
  if (has_time_a && has_time_b) {
    ...
    ok_a = client_handlers_[cid_a]->requestSubmapByTime(t1, cid_a, &submap_id_a,
                                                        &submap_a, &T_A_t1);
    ok_b = client_handlers_[cid_b]->requestSubmapByTime(t2, cid_b, &submap_id_b,
                                                        &submap_b, &T_B_t2);
...
void CoxgraphServer::addToMFFuture(
  ...
  if (map_fusion_msgs_future_.size() > config_.map_fusion_queue_size) {
    ...
    LOG_IF(INFO, verbose_)
        << "Future map fusion too many, dropping the last msg";
  }
}
...
bool CoxgraphServer::fuseMap(const CliId& cid_a, const ros::Time& t1,
  ...
  Transformation T_Cli_SmA = cli_map_collection_ptr_->addSubmap(submap_a);
  Transformation T_Cli_SmB = cli_map_collection_ptr_->addSubmap(submap_b);
  ...
  Transformation T_CliA_CliB =
      T_Cli_SmA * T_A_t1 * T_t1_t2 * T_B_t2.inverse() * T_Cli_SmB.inverse();
  pose_graph_interface_.addLoopClosureMeasurement(
      submap_a->getID(), submap_b->getID(), T_CliA_CliB);
